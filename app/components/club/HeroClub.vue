<template>
	<section class="relative pt-32 pb-20 bg-white overflow-hidden">
		...
		<!-- Éléments de fond (Montagnes & Lignes) -->
		<div class="absolute inset-0 pointer-events-none overflow-hidden">

			<!-- Montagnes stylisées (SVG) avec animation -->
			<svg
					ref="mountain1"
			...
			<!-- Seconde couche montagne plus claire avec animation -->
			<svg
					ref="mountain2"
			...
			<!-- Lignes courbes décoratives (Top) avec animation -->
		...
		<UContainer class="relative z-10 text-center">
			...
			<p ref="subtitle" class="text-gray-500 text-lg md:text-xl font-medium max-w-2xl mx-auto mb-8 opacity-0">
				Plus qu'une association, une famille de grimpeurs.
			</p>

			<!-- Trait décoratif vert avec animation -->
...
onMounted(() => {
	...
	// Animation des courbes SVG (effet de dessin)
	if (curve1.value) {
	...
	// Animation des montagnes (parallax léger au scroll)
	if (mountain1.value) {
	...
	// Animation des cercles concentriques (pulsation)
	...
	// Animation des points décoratifs (apparition séquentielle)
...
<style scoped>
/* Styles pour assurer la fluidité */
svg {
