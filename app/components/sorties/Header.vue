<template>
	<section class="relative pt-32 pb-20">
		...
		<div class="absolute inset-0 pointer-events-none ">
			...
			<!-- Vagues stylisées (différentes du club) -->
			<svg
					ref="wave1"
		...
		<UContainer class="relative z-10 text-center">
			...
			<p ref="subtitle" class="text-gray-500 text-lg md:text-xl font-medium max-w-2xl mx-auto mb-8 opacity-0">
				Vivez l'escalade grandeur nature en falaise
			</p>
...
onMounted(() => {
	...
	// Animation des chemins (sentiers)
	if (path1.value) {
		gsap.to(path1.value, {
			strokeDashoffset: 0,
			duration: 2.5,
	...
	// Animation des rochers (rotation)
	if (rock1.value) {
		gsap.to(rock1.value, {
			rotation: 10,
