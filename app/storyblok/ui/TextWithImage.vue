    ...
    <UContainer>
      ...
      >
        ...
        <div class="flex flex-col gap-6 self-start justify-self-start w-full" :class="{ 'lg:order-2': blok.reverse_layout }">
          ...
          <!-- Contenu texte riche (généré par Storyblok) -->
          <div 
        ...
        <!-- Colonne Visuel (on la laisse centrée elle-même pour l'image) -->
        <div class="relative h-[400px] w-full bg-gray-50 rounded-3xl overflow-hidden flex items-center justify-center group" :class="{ 'lg:order-1': blok.reverse_layout }">
          ...
          <template v-else>
            ...
            <div class="relative z-10 flex flex-col items-center">
              ...
              <span class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-lg font-bold text-[#0F1729] bg-white px-4 py-1 rounded-full shadow-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300">{{ blok.year_label || 'Création' }}</span>
            </div>
          </template>
